...
from tf_transformations import (
    ...
    translation_from_matrix, inverse_matrix, concatenate_matrices
)
...
class PDMotionPlanner(Node):
    def __init__(self):
        ...
        self.path_sub = self.create_subscription(Path, "/time_trajectory", self.path_callback, 10)
    ...
    def control_loop(self):
        ...
        # Transform global path to robot frame if needed
        ...
        if distance <= 0.1:
            if not self.goal_reached:
                ...
                stop_cmd = Twist()
                self.cmd_pub.publish(stop_cmd)  # Stop robot
        ...
        # Relative transform (robot → next goal)
    ...
    # ----------------------------
    # Utility: Choose next waypoint
    # ----------------------------
    def get_next_pose(self, robot_pose: PoseStamped) -> PoseStamped:
        """Return the next pose at least step_size away."""
    ...
    # ----------------------------
    # Utility: Transform global path
    # ----------------------------
    def transform_plan(self, frame):
        """Transform global path to target frame."""
        ...
        try:
            ...
            self.get_logger().error(
                f"Couldn't transform plan from {self.global_plan.header.frame_id} → {frame}: {ex}")
            return False
